      ...
      <div className='w-[60%] h-full  flex gap-5 '>
        ...
        <div className='w-[40%] h-full flex flex-col gap-3'>
          ...
          <div className='w-[110px] h-[50px] text-orange-500 border-2 text-xl border-orange-500 bg-slate-400 flex rounded-lg overflow-hidden shadow-lg'>
            <button className='w-[30%] h-full hover:bg-slate-100 bg-white cursor-pointer flex justify-center items-center' onClick={() => { qty > 1 ? dispatch(DecrementQty(id)) : 1 }} >-</button>
            <span className='w-[40%] h-full  bg-slate-200 flex justify-center items-center'>{qty}</span>
            <button className='w-[30%] h-full hover:bg-slate-100 bg-white cursor-pointer flex justify-center items-center' onClick={() => { dispatch(IncrementQty(id)) }}>+</button>
          </div>
        </div>
      </div>
      <div className='flex flex-col items-end gap-10'>
        <span className='text-xl text-orange-500 font-semibold'>{price}</span>
        <RiDeleteBin6Line size={20} className='cursor-pointer' onClick={() => { dispatch(RemoveItem(id)); toast.error("item removed") }} />
      </div>
    </div>
  )
}
